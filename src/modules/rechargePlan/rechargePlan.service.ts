...
export const RechargePlanService = {
  ...
  async updatePlan(id: number, data: any, adminId: string) {
    ...
    const mergedData = {
      ...
      validityDays:
        data.validityDays !== undefined ? data.validityDays : normalizedOld.validityDays,
      ...
      monthlyPrice:
        data.monthlyPrice !== undefined ? data.monthlyPrice : normalizedOld.monthlyPrice,
  ...
  async notifyExpiringSubscribers() {
    ...
    for (const driver of allActiveDrivers) {
      try {
        ...
        logger.error(
          `Failed to notify driver ${driver.driver_id}: ${err.response?.data?.message || err.message}`
        );
      }
    }
  ...
  async notifyAllExpiredSubscribers() {
    ...
    for (const driver of allExpiredDrivers) {
      try {
        ...
        logger.error(
          `Failed to notify expired driver ${driver.driver_id}: ${err.response?.data?.message || err.message}`
        );
      }
    }
  ...
  async runAutomatedDailyNotifications() {
    ...
    for (const driver of allDrivers) {
      try {
        ...
        logger.error(
          `Failed automated notification for driver ${driver.driver_id}: ${err.message}`
        );
      }
    }
